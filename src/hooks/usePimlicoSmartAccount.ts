...
/**
 * Hook to create Simple Smart Account with Pimlico for gasless transactions
 * Using Simple Smart Account (not Safe)
 */
export const usePimlicoSmartAccount = () => {
  ...
  useEffect(() => {
    const initializeClient = async () => {
      ...
      try {
        console.log("🚀 Setting up Simple Smart Account with Pimlico...");
        console.log("Para Wallet Owner:", walletClient.account?.address);

        // Create Simple Smart Account with Para wallet as owner
        // Following Para team's working example pattern
        const account = await toSimpleSmartAccount({
          client: publicClient,
          owner: walletClient.account,  // Para wallet is the owner
          entryPoint: {
            address: entryPoint07Address,
            version: "0.7"
          },
        ...
        console.log("✅ Simple Smart Account created:", account.address);
        ...
        console.log("🎉 Gasless transactions enabled via Pimlico!");

        setSmartAccount(account);
        setSmartAccountClient(client);
        setError(null);
      } catch (error: any) {
        console.error("❌ Failed to initialize Simple Smart Account:", error);
        setError(error?.message || "Failed to initialize Smart Account");
